...
// Binary value to generate desired voltage with respect to VBIN = int { (1/SLOPE)*(desired_voltage+VMID) + VMIN/SLOPE }
...
// ADC VOLTAGE MEASURED = (ARDUINO_SUP PLY_VOLTAGE/4095)*ADC_BITS
// MEASUREMENT SHOULD NOW BE ACCURATE TO WITHIN A 1mV OR SO

// IMPORTANT: WHEN USING GRAPHENE SAMPLES, COMPILE & UPLOAD BEFORE INSERTING GRAPHENE CHIP. DURING UPLOADING SPURIOUS VDS SIGNALS ARE GENERATED WHICH MAY DAMAGE/DESTROY GRAPHENE FETS

// IMPORTANT: Peripheral IdentifiePYrs on page 38 (PIOC, DACC, etc)

float VGS_SWEEP_RATE = 500;  // 1000 = 1s, VGS sweep rate in ms for IV
float VDS_HOLD = 10000;       // 30,000 = 30s, VDS_hold at beginning of sweep
...
float VDS_START = 900e-3;
float VDS_STOP = 900e-3;
float VDS_INC = 5e-3;
float VDS;
int VDS_bin;
String message;
...
void loop() {
  ...
  for(VDS = VDS_START; VDS <= VDS_STOP+0.5*VDS_INC; VDS = VDS + VDS_INC) {    // added 0.5*VDS_INC just in case VDS is not exactly precise and the loop terminates early
    ...
    for(VGS = VGS_START; VGS <= VGS_STOP+0.5*VGS_INC; VGS = VGS + VGS_INC) {  // added 0.5*VGS_INC just in case VDS is not exactly precise and the loop terminates early
...
String sweep(float VDS, float VGS) {
  ...
  /*
  PIOC -> PIO_ODSR = 0b00000000000000000000000000000000;
  ...
  */
  ...
  for (VDS_select = 0; VDS_select < 16; VDS_select++) {   
    for (VOUT_select = 0; VOUT_select < 16; VOUT_select++) {      
      PIOC -> PIO_ODSR = (VDS_select << 16) | (VOUT_select << 12) | (768);	              // write VDS select bits and VOUT select bits
...
/*int VBIN(float desired_voltage) {
  int binary_value;
  binary_value = int(1879.23148 * desired_voltage + 1810.3955);
  return binary_value;
}
*/
