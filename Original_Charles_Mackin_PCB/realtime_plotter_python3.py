...
s = serial.Serial(port='/dev/cu.usbmodem14101', baudrate=115200)	    # Arduino DUE Programming Port
...
def get_data(time, VDS, VGS, VMID, data, R):	
	...
	if(len(string.decode().split(','))%260 == 0):
		...
		data = data/GAIN
		data = np.reshape(data,(16,16))							# reshape array to 16 x 16
		#data = np.ones((16,16))*100e-6
		VDS_array = np.ones((16,16))*VDS
...
def run():
	...
	for axis in [ax.xaxis, ax.yaxis]:	# Set the major ticks at the centers and minor tick at the edges
		axis.set_ticks(np.arange(16) + 0.5, minor=True)
	...
	plt.xticks(np.arange(0, 16, 1), range(16))		# labels every node in the array along the x and y axis
	plt.yticks(np.arange(0, 16, 1), range(16))
	...
	while True:
		...
		try:
			...
			device_yield = 100*np.true_divide(working_devices,256)
