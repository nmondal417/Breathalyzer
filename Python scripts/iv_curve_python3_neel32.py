...
#s = serial.Serial(port='/dev/cu.SLAB_USBtoUART', baudrate=115200)	# Arduino DUE
...
def get_data(time, VDS, VGS, VMID, data, R, string):	
	...
	if(len(string.decode().split(','))%1028 == 0):
		...
		time = data[0]
		VDS = data[1]
		VGS = data[2]*multiplexn
		VMID = data[3]
		#data = data[4:]*BITS_TO_V
		data = data[4:]
		data = data/GAIN
		#print(len(data))
		data = data.reshape(1024,1)							# reshape array to 16 x 16
		VDS_array = np.ones((1024,1))*VDS
...
def run():
	...
	while True:
		...
		try:
			...
			if('END' in str(string) or RECORD == 1):
				...
				np.savez(data_file_name, time_stamps=time_stamps, VDS_2d=VDS_1d, VGS_2d=VGS_1d, data_3d=data_2d)
			...
			if RECORD == 0:
				...
				print("VDS = %0.2f mV,  VGS = %0.2f  V,  VMID_bin = %0.2f bits" %(1e3*VDS, VGS, VMID))
				...
				#print(len(data))
				data_2d = np.hstack((data_2d,data))

				plt.title(r'$\mathrm{V_{DS}\ =\ %0.4f\ V}$' %VDS, y=1.01, fontsize=FONTSIZE) 
				for i in range(np.int(data_2d.shape[0] / 2)):
					if(i % 32 == 0):													# reduce number of lines to speed up plotting, make sure python doesn't lag behind actual measurement
				...
				#	data_2d = np.empty((1024,0))				# clear 2d array
			...
			#if(len(string.decode().split(',')) == 3 and '1024' in str(string)):
